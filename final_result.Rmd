...
metrics %>% 
  arrange(.metric, desc(mean))
```
Niskie wyniki kappa co sugeruje nam już, że modele miały problem z przewidywaniem. Mimo to nie zrażamy się i wybieramy modele najlepsze pod względem metryki mn_log_loss.

### Trenowanie modeli z najlepszymi parametrami
...
#### Deklaracja workflowów oraz ostateczne uczenie
...
#### Predykcje ostatecznych modeli na zbiorze testowym i treningowym
...
#### Macierze pomyłek dla ostatecznych modeli
...
Raport okazał się bardziej objętościowy niż początkowo planowaliśmy i dlatego nie zamieszczaliśmy części mniej znaczącego kodu skupiając się bardziej na samych wynikach (include = FALSE).
